...
Once YAX has been installed, you can quickly get started by running  ` ./yax cis --help`. 
## Overview
You can use YAX for cis-xQTL analysis using either a) "vanilla" analysis ordinary least squares (OLS) regression, or b) a linear mixed model (LMM) to account for cryptic or familial relatedness fit by restricted maximum likelihood (REML).  For OLS, YAX requires 3 input files: molecular trait data, technical covariate data, and genotype data. For LMM, YAX additionally requires a kinship or genetic relatedness matrix (GRM). For detailed descriptions of input file formats, please see the [input file documentation page](#input-format). 
